...
Look for NDA screening criteria in local settings (e.g., a `legal.local.md` file).
...
**Redline approach**: Delete the provision entirely. If the counterparty insists, limit to targeted solicitation (not general recruitment) and set a short term (12 months). Note that the enforceability of restrictive covenants varies significantly by jurisdiction.
