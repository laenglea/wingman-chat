...
task tracker:  text search "API migration" in relevant workspace
```
...
| **Document**    | "Where's the spec for Z?"         | Prioritize cloud storage, wiki, shared docs                         |
...
Map enterprise-style filters onto whatever search syntax each source supports.
...
query: "from:sarah aurora"
```

**Filter mapping:**
| Generic filter | Typical chat syntax |
|------------------|--------------|
| `from:sarah` | `from:sarah` (or a user ID) |
...
assignee: "me"  (for "my tasks" queries)
```

**Filter mapping:**
| Generic filter | Typical task tracker parameter |
|------------------|----------------|
| `from:sarah` | assignee or created-by filter |
| `after:2025-01-01` | modified-after filter |
| `type:milestone` | resource subtype filter |
...
[User query]
     ↓ decompose
[chat query] [email query] [cloud storage query] [wiki query] [task tracker query]
     ↓            ↓            ↓              ↓            ↓
  (parallel execution)
     ↓
