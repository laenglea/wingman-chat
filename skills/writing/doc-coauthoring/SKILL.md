...
3. **Reader Testing**: Test the doc with a fresh reader (no context) to catch blind spots before others read it
...
- If images exist without alt-text, explain that when others use an assistant to understand the doc, the assistant won't be able to see them. Ask if they want alt-text generated. If so, request they paste each image into chat for descriptive alt-text generation.
...
**Goal:** Test the document with a fresh reader (no context bleed) to verify it works for readers.
...
1. Open a fresh assistant conversation
