 ...
 */
export function getLineRange(lines: string[], startLine: number, endLine?: number): string[] {
  ...
  const end = endLine !== undefined ? Math.max(start, Math.min(endLine, total)) : total;
...
export function grepText(
  ...
  const { caseSensitive = false, ignoreCase, literal = false, maxMatches = 50, contextLines = 0 } = options;
