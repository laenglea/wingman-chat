...
function processShapeTree(element: Element, relationships: Relationships, callback: (info: ShapeInfo) => void): void {
...
function parseShape(sp: Element, relationships: Relationships): ShapeInfo {
  ...
  const isTitle = phType === "title" || phType === "ctrTitle" || shapeName.includes("title");
...
function parseTextRun(r: Element, relationships: Relationships): string {
  ...
  const isStrike = rPr?.getAttribute("strike") === "sngStrike" || rPr?.getAttribute("strike") === "dblStrike";
...
function parseGraphicFrame(graphicFrame: Element, relationships: Relationships): string | null {
  ...
  for (const row of rows) {
    ...
    for (const cell of cells) {
      ...
      let cellText = "";
      if (txBody) {
        cellText = parseTextBody(txBody, relationships).replace(/\n/g, "<br>").trim();
      }
