...
export function WizardStepIndicator({ steps, currentStep, visitedSteps, onStepClick }: WizardStepIndicatorProps) {
  return (
    <div className="flex items-center justify-center gap-0.5 px-4 py-2.5 border-b border-neutral-200/60 dark:border-neutral-800/60 overflow-x-auto">
      {steps.map((step, i) => {
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium transition-colors ${
                isActive
                  ...
                  : isCompleted
                    ? "text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800/60 cursor-pointer"
                    : "text-neutral-300 dark:text-neutral-600 cursor-default"
              }`}
            >
              {isCompleted ? <Check size={12} /> : <step.icon size={12} />}
              <span className={isActive ? "inline" : "hidden"}>{step.label}</span>
            </button>
          </div>
        );
      })}
