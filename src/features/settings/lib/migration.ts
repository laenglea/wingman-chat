  ...
  for (const image of oldImages) {
    ...
    try {
      ...
      const meta = {
        id: image.id,
        title: image.title,
        created: image.created instanceof Date ? image.created.toISOString() : image.created,
        updated: image.updated instanceof Date ? image.updated.toISOString() : image.updated,
        model: image.model,
        prompt: image.prompt,
      };
